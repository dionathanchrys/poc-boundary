...
kubectl wait -f app/boundary-postgres/base/deployment.yaml --for condition=Available --timeout 120s -n boundary --context kind-svc-cluster
...
kubectl wait -f app/boundary-migrator/base/job.yaml --for condition=Complete --timeout 120s -n boundary --context kind-svc-cluster
echo " " && echo "➡ ➡ ➡ ➡ BOUNDARY ADMIN CREDENTIALS ⬅ ⬅ ⬅ ⬅" && echo " "
echo "####################################################"
echo "####                                            ####"
echo "####   Login:              admin                ####"
echo "####                                            ####"
echo "#### $(kubectl logs jobs/boundary-migrator -n boundary --context kind-svc-cluster | grep "Password:") ####"
echo "####                                            ####"
echo "####                                            ####"
echo "####################################################"
echo "  Login:              admin" > boundary-credential.txt
echo "$(kubectl logs jobs/boundary-migrator -n boundary --context kind-svc-cluster | grep "Password:")" >> boundary-credential.txt && echo "Boundary credentials saved to boundary-credential.txt"
...
kubectl wait -f app/boundary-controller/base/deployment.yaml --for condition=Available --timeout 120s -n boundary --context kind-svc-cluster
...
echo " " && echo "➡ ➡ ➡ ➡ DEPLOYING BOUNDARY WORKER- PRD ENV ⬅ ⬅ ⬅ ⬅" && echo " "
